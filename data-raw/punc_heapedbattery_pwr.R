## code to prepare `punc_heapedbattery_pwr` dataset goes here

system.time(
  punc_heapedbattery_pwr <- puncture_lm_sim(
    n = 35,
    sim_iter = 500,
    beta_gen = function() {
      list(
        0,    # beta0 (intercept)
        0.30, # beta1 (power effect)
        0.50  # beta2
      )
    },
    gen_ivs = function(n) {
...
usethis::use_data(punc_heapedbattery_pwr, overwrite = TRUE)
