...
punc_type1_1 <- puncture_lm_sim(
  ...
  gen_ivs = function(n){
    ...
    # Define correlation matrix with stronger correlations
    # Using stronger correlations between auxiliary vars and main predictors
    R <- matrix(c(
      1.0,  0.2,  0.6,  0.7,  0.5,  # correlations with X1 (increased)
      0.2,  1.0,  0.7,  0.5,  0.6,  # correlations with X2 (increased)
      0.6,  0.7,  1.0,  0.3,  0.3,  # correlations with X3 (increased)
      0.7,  0.5,  0.3,  1.0,  0.3,  # correlations with X4 (increased)
      0.5,  0.6,  0.3,  0.3,  1.0   # correlations with X5 (increased)
    ), nrow = 5, byrow = TRUE)
