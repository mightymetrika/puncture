# data-raw/punc_latentproxy_type1.R

## code to prepare `punc_latentproxy_type1` dataset goes here

system.time(
  punc_latentproxy_type1 <- puncture_lm_sim(
    n = 25,
    sim_iter = 500,
    beta_gen = function() {
      list(
        0,    # beta0 (intercept)
        0,    # beta1 (target null for Type I)
        0.45  # beta2
      )
    },
    gen_ivs = function(n) {
      ...
      # This creates confounding pressure if severity is not adjusted well.
      X1 <- std(
        0.70 * theta +
      ...
      # This is the covariate the standard regression must rely on.
      ...
      # This is what makes the noisy X2 an imperfect adjustment set.
      epsilon_pop <-
        0.70 * theta +
    ...
    mpat = function(mdat) {
      ...
      # This makes the imputation model work hardest exactly where the nuisance
...
usethis::use_data(punc_latentproxy_type1, overwrite = TRUE)
