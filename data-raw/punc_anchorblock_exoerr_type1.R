## code to prepare `punc_anchorblock_exoerr_type1` dataset goes here

system.time(
  punc_anchorblock_exoerr_type1 <- puncture_lm_sim(
    n = 25,
    sim_iter = 500,
    beta_gen = function() {
      list(
    ...
    gen_ivs = function(n) {
      ...
      # Still difficult structure like punc_anchorblock_type1, but error term is exogenous
      epsilon_pop <-
        0.80 * theta +
        stats::rnorm(N_pop, mean = 0, sd = 0.60 + 0.50 * rare_grp)
...
usethis::use_data(punc_anchorblock_exoerr_type1, overwrite = TRUE)
