# data-raw/punc_recruit_type1_med.R

## code to prepare `punc_recruit_type1_med` dataset goes here

system.time(
  punc_recruit_type1_med <- puncture_lm_sim(
    n = 25,
    sim_iter = 500,
    beta_gen = function() {
      list(
    ...
    mpat = function(mdat) {
      ...
      c1 <- stats::rbinom(n, 1, 0.80)
      c2 <- stats::rbinom(n, 1, 0.80)
      pattern_vec <- c1 * c2
...
usethis::use_data(punc_recruit_type1_med, overwrite = TRUE)
